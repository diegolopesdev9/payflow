...
export class SupabaseStorage implements IStorage {
  ...
  async getAggregatedStats(): Promise<any> {
    console.log('📊 [getAggregatedStats] Buscando estatísticas agregadas');
    ...
    try {
      ...
      // Usuários ativos (30-60 dias atrás) para comparação
      ...
      // Tempo médio desde cadastro (em dias)
      ...
      // Taxa de retenção (usuários que ainda estão ativos após 30 dias)
      const retentionRate = totalUsers && totalUsers > 0
        ? ((totalUsers - (suspendedUsers || 0)) / totalUsers) * 100
        : 100;

      console.log('✅ [getAggregatedStats] Stats agregadas calculadas');
      ...
      console.error('❌ [getAggregatedStats] Erro:', error);
      throw error;
    }
  }

  async exportUsersData(): Promise<any[]> {
    console.log('📥 [exportUsersData] Exportando dados de usuários');
    ...
    try {
      ...
      // Buscar contagem de contas para cada usuário
      ...
      console.log('✅ [exportUsersData] Dados exportados:', usersWithCounts.length, 'usuários');
      return usersWithCounts;
    } catch (error: any) {
      console.error('❌ [exportUsersData] Erro:', error);
      throw error;
    }
  }
}
